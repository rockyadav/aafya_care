                        ...
                        <table id="userdatatables" class="table table-striped table-no-bordered table-hover" cellspacing="0" width="100%" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="td-actions">
                                @if($row->status=="Submitted in Lab" || $row->status=="Report Generated")
                                         <a href="{{url('admin/laboratory-generate-report/'.base64_encode($row->id))}}">
                                            <button type="button" rel="tooltip" class="btn btn-success btn-sm" data-original-title="Generate Report" title=""><i class="material-icons">edit</i>
                                            <div class="ripple-container"></div></button>
                                        </a> 
                                        @endif

                            @if($row->status=="Sample Taken" || $row->status=="Submitted In Laboratory")
									 ...
									 <a href="{{url('admin/laboratory-customer-details/'.base64_encode($row->id))}}">
...
<div id="changeStatusModal" class="modal model-wide fade" data-keyboard="false" data-backdrop="static" role="dialog">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
           ...
           <form method="post" class="take-sample-formsss" action="{{url('admin/SubmittedLaboratory')}}" class="form-horizontal" accept-charset="UTF-8" role="form" enctype="multipart/form-data">  
            {{ csrf_field() }}    
              <input type="hidden" name="user_id" id="to_user" value="" required>      
                    ...
                    <select class=" form-group selectpicker" data-style="select-with-transition" title="Choose course" data-size="7" name="sample_status" id="sample_status" data-live-search="true" required="true" tabindex="-98">
                       <option value="" disabled="">Choose status </option>
                        <option value="Submitted in Lab">Submitted in Lab</option>
